...
def run_case(name: str, x_sparse: torch.Tensor, w: torch.Tensor, agemm) -> None:
    m, k = x_sparse.shape
    n = w.shape[0]
    reorder_index = torch.arange(k, device=x_sparse.device, dtype=torch.int16)

    qx_sparse, sfx_sparse = agemm.reorder_quantize_x(x_sparse, reorder_index, 0)
    qw32, sfw_sparse32, sfw_dense16 = agemm.quantize_w32_shared(w)
    a_comp, e = agemm.compress_sparse_a(qx_sparse, n)

    y_dense = agemm.matmul(qx_sparse, qw32, sfx_sparse, sfw_dense16, 1.0).float()
    y_sparse = agemm.sparse_matmul(a_comp, qw32, e, sfx_sparse, sfw_sparse32, m, n, k).float()
...
def main():
    ...
    run_case("shared-w32: x_sparse x eye", x_sparse, eye, agemm)
    print()
    run_case("shared-w32: x_sparse x rand_w", x_sparse, rand_w, agemm)
    print()
    run_case("shared-w32: x_sparse x partially_tied_w", x_sparse, block_scaled_w, agemm)
