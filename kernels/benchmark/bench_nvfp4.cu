...
int main() {
    const int M = 4096;
    const int N = 4096;
    const int K = 4096 * 1.4375;
    const int block_size = 16;
    const int warm_up = 2000;
    const int num_bench = 4000;
    ...
    for (int it = 0; it < warm_up; it ++) {
        matmul_host_nvfp4_bf16(A_d, B_d, M, N, K, C_d, D_d, SFA_d, SFB_d, 1.0);
    }
    CHECK_CUDA(cudaEventRecord(start));
    for (int it = 0; it < num_bench; it ++) {
        matmul_host_nvfp4_bf16(A_d, B_d, M, N, K, C_d, D_d, SFA_d, SFB_d, 1.0);
    }
    ...
    std::printf("GEMM completed in %.6f ms\n", milliseconds / num_bench);
